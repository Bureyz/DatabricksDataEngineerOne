...
AS SELECT * FROM cloud_files('${source_path}/Customers.csv', 'csv', map("header", "true", "inferSchema", "true"));

-- Bronze Products
CREATE OR REFRESH STREAMING TABLE bronze_products
COMMENT 'Raw products data from CSV'
AS SELECT * FROM cloud_files('${source_path}/Product.csv', 'csv', map("header", "true", "inferSchema", "true"));

-- Bronze Orders
CREATE OR REFRESH STREAMING TABLE bronze_orders_header
COMMENT 'Raw orders data from CSV'
AS SELECT * FROM cloud_files('${source_path}/SalesOrderHeader.csv', 'csv', map("header", "true", "inferSchema", "true"));

-- Bronze Orders
CREATE OR REFRESH STREAMING TABLE bronze_orders_detail
COMMENT 'Raw orders data from CSV'
AS SELECT * FROM cloud_files('${source_path}/SalesOrderDetail.csv', 'csv', map("header", "true", "inferSchema", "true"));
...
AS SELECT * FROM cloud_files('${source_path}/ProductCategory.csv', 'csv', map("header", "true", "inferSchema", "true"));
